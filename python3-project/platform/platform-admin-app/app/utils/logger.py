...
# 支持多应用独立配置（如管理端与应用端运行在同一进程时互不干扰）
...
def setup_logging(
    ...
    参数:
        ...
        log_dir: 日志文件目录。None 或空则禁用文件日志。
        log_file: log_dir 中的日志文件名。
        app_name: 应用标识，用于区分不同应用的日志配置。
        json_format: 是否使用 JSON 结构化格式（生产环境推荐）。
    """
